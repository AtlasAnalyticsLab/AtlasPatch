...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SAM2 + Optuna HPO for Virya")
    parser.add_argument("--slurm", type=str, default=None, help="Path to SLURM_TMPDIR")
    args = parser.parse_args()

    with open("config/hpt_cluster_config.yaml", 'r') as config_file:
        config = yaml.safe_load(config_file)

    train_data_path = config['train_data_path']
    validation_data_path = config['validation_data_path']
    if args.slurm:
        train_data_path = train_data_path.replace("$TMPDIR/src", args.slurm)
        validation_data_path = validation_data_path.replace("$TMPDIR/src", args.slurm)
