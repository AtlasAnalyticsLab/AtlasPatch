...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Training script with optional SLURM support.")
    parser.add_argument("--slurm", type=str, default=None, help="Path to SLURM_TMPDIR if running on a SLURM job.")
    ...
    # Assign parameters from config
        train_data_path = config['train_data_path'].replace("$TMPDIR/src", args.slurm)
        validation_data_path = config['validation_data_path'].replace("$TMPDIR/src", args.slurm)
    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        if (epoch + 1) % 5 == 0:
            SaveModel(predictor.model, optimizer, epoch + 1, saved_model_name)
